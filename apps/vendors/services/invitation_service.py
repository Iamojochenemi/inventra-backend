...
from apps.vendors.models import Branch, VendorInvitation, VendorStaff
...
def create_invitation(vendor, email, role, created_by, request=None):
    ...
    try:
        ...
        existing_staff = VendorStaff.objects.filter(
            vendor=vendor, user__email=email
        ).exists()
        ...
        existing_invitation = VendorInvitation.objects.filter(
            vendor=vendor, email=email, status="pending"
        ).first()
...
def send_invitation_email(invitation, request=None):
    ...
    try:
        ...
        html_message = render_to_string("emails/vendor_invitation.html", context)
    ...
    try:
        ...
        if not main_branch:
            main_branch = Branch.objects.create(
                vendor=invitation.vendor, name="Main Branch"
            )
        ...
        logger.info(
            f"Invitation accepted for {invitation.email} to {invitation.vendor.name}"
        )
