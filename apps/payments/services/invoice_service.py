...
def create_invoice_from_order(order, notes=""):
    ...
    try:
        ...
        invoice = Invoice.objects.create(
            ...
            currency=(
                order.vendor.settings.currency
                if hasattr(order.vendor, "settings")
                else "NGN"
            ),
...
def generate_invoice_pdf(invoice):
    ...
    try:
        ...
        details = [
            ...
            [
                "Invoice Date:",
                invoice.issued_at.strftime("%Y-%m-%d") if invoice.issued_at else "N/A",
            ],
            [
                "Due Date:",
        ...
        items_table = Table(
            items_data, colWidths=[2.5 * inch, 1.2 * inch, 1.2 * inch, 1.2 * inch]
        )
        ...
        story.append(
            Paragraph(
                f"<b>Total: {invoice.currency} {invoice.amount:,.2f}</b>", total_style
            )
        )
...
def mark_invoice_as_overdue(invoice):
    ...
    if (
        invoice.due_date
        and timezone.now() > invoice.due_date
